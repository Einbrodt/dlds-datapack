...
data modify storage dlds:reward item1 set value {name:"Unheilvolle Flasche Drohendes Urteil V",Count:1}
data modify storage dlds:reward item2 set value {name:"Erfahrungspunkte",Count:25}


give @s minecraft:ominous_bottle[ominous_bottle_amplifier=4] 1
experience add @s 25 points

execute as @a run function dlds:display_reward
tag @s add dragon_breath_reward
